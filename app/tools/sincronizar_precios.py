def sincronizar_precios_meli_sheets() -> str:
    ...
    y corrige Siigo, Sheets y el catálogo web donde difieran. Antes esta función
    solo tocaba Sheets y borraba el cache web sin arreglar Siigo — como el
    catálogo web y las cotizaciones de los agentes de atención al cliente leen
    el precio en vivo de Siigo, eso dejaba las cotizaciones mal aunque MeLi
    ...
    if not candidatos:
        return "✅ Precios ya están sincronizados entre MeLi, Siigo, Sheets y la web. Sin cambios."

    salida = [f"🔄 {len(candidatos)} producto(s) con precio distinto entre MeLi y Siigo:"]
    for c in candidatos:
        ok = (c.get("siigo_resultado") or {}).get("ok")
        marca = "✅" if ok else "❌"
        salida.append(
            f"{marca} {c['nombre'][:45]} (SKU {c['sku']}): "
            f"Siigo ${c['precio_siigo_antes']:,.0f} → ${c['precio_meli']:,.0f} (precio actual en MeLi)"
        )

    salida.append(f"\n✅ Aplicados en Siigo: {resultado['aplicados']}/{len(candidatos)}")
