    ...
    with _conn() as db:
        ...
        if nuevo_estado == "resuelto":
            ...
            try:
                ...
                from app.observability import spawn_thread
                spawn_thread(
                    notificar_revision_solicitada, (ticket_id, uid), daemon=True,
                )
            except Exception:
                pass
        elif t["estado"] == "resuelto" and nuevo_estado in ("en_proceso", "pendiente") and t["tipo"] == "solicitud":
            # Reapertura: botón explícito "Reabrir solicitud" o el seguimiento implícito
            # de HistorialSeguimientoChat — mismo mecanismo, un solo lugar que notifica.
            try:
                from app.services.tickets_notificaciones import notificar_ticket_reabierto
                from app.observability import spawn_thread
                spawn_thread(
                    notificar_ticket_reabierto, (ticket_id, uid), daemon=True,
                )
            except Exception:
                pass
