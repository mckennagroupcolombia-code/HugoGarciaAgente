...
# --- IMPORTACIONES CORE ---
from core_sync import GOOGLE_CREDS_PATH, refrescar_token_meli, ejecutar_sincronizacion_y_reporte
...
# Configurar API de Google Gemini (Lee la API Key desde .env)
...
def enviar_email_reporte(destinatario: str, asunto: str, cuerpo: str):
    """Envía un correo real usando SMTP de forma segura."""
    print(f"📧 [EMAIL] Enviando reporte a {destinatario}...")
    remitente = "mckenna.group.colombia@gmail.com"
    password = os.getenv("EMAIL_PASSWORD")

    if not password:
        return "❌ Error Crítico: La contraseña de email no está configurada en el archivo .env. Notifique al administrador."
...
def leer_datos_hoja(producto_buscar: str):
    """Lee inventario desde Google Sheets de forma segura."""
    try:
        SPREADSHEET_ID = os.getenv("SPREADSHEET_ID")
        if not SPREADSHEET_ID:
            return "❌ Error Crítico: El SPREADSHEET_ID no está configurado en el archivo .env. Notifique al administrador."
