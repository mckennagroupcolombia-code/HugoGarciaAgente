...
- **Studio web scroll:** el hub `studio-web` debe usar el mismo fill+overflow-hidden que Contabilidad. Si el wrapper es `overflow-y-auto`, el canvas alto se traga Guardar/Publicar. `PanelTransition` fillHeight incluye `sitioweb`. Acciones viven en `#studio-web-chrome` (portal junto al título del Layout), no duplicadas en la barra del lienzo.
