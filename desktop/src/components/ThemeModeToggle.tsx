...
export default function ThemeModeToggle({
  ...
  return (
    <button
      ...
      aria-pressed={dark}
      aria-label={label}
      className={`mck-press inline-flex h-10 w-10 items-center justify-center rounded-full border-2 border-border bg-surface-panel text-ink shadow-paper-sm transition-colors hover:border-accent hover:bg-accent hover:text-white ${className}`.trim()}
    >
      {dark ? (
        <Icon name="sun" size={20} weight="bold" />
      ) : (
        <Icon name="moon" size={20} weight="bold" />
      )}
    </button>
  );
}
