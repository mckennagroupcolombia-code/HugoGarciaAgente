  ...
  return (
    <div className="mx-auto max-w-3xl space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={() => refetch()}
          className="rounded-lg border border-border px-3 py-1.5 text-xs text-muted transition hover:text-ink"
        >
          Actualizar
        </button>
      </div>
      ...
      {!isLoading && mensajes.length === 0 && (
        <div className="rounded-xl border border-border bg-surface-panel p-8 text-center">
          <p className="text-sm text-success">Sin mensajes postventa pendientes</p>
        </div>
      )}
      ...
      {mensajes.map((m) => (
        ...
        >
          ...
          {errores[m.codigo] && (
            <p className="text-xs text-danger">{errores[m.codigo]}</p>
          )}
