...
function PerfilContent({
  ...
  const [documento, setDocumento] = useState(user.documento_identidad || "");
  ...
  }, [user.nombre, user.documento_identidad]);
  ...
  async function guardar(ev: React.FormEvent) {
    ...
    if (!doc) {
      setMsg({
        type: "err",
        text: "El documento de identidad es obligatorio para generar cuentas de cobro.",
      });
      return;
    }
    if (doc.length < 5 || !/\d/.test(doc)) {
      setMsg({ type: "err", text: "Documento de identidad inválido." });
      return;
    }
    ...
    try {
      ...
      const body: { nombre: string; documento_identidad: string; password?: string } = {
        nombre: nombre.trim(),
  ...
  const docGuardado = Boolean((user.documento_identidad || "").trim());
  ...
  return (
    <div className="w-full max-w-none space-y-5">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        {!docGuardado && (
          <p className="rounded-paper border border-amber-400/60 bg-amber-50 px-3 py-1.5 text-xs font-semibold text-amber-800 dark:bg-amber-950/30 dark:text-amber-200">
            Falta documento de identidad — requerido para cuentas de cobro
          </p>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 gap-5 xl:grid-cols-12">
        ...
        <div className="space-y-5 xl:col-span-5">
          <div className="rounded-paper border-2 border-border bg-surface-panel p-5 shadow-paper-sm">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
              ...
              <div className="min-w-0 flex-1">
                <p className="font-extrabold text-ink text-lg">{nombre || user.nombre}</p>
                ...
                {(user.documento_identidad || documento) && (
                  <p className="mt-1 font-mono text-xs text-ink-secondary">
                    CC/NIT: {user.documento_identidad || normalizarDocumento(documento) || "—"}
                  </p>
                )}
          ...
          <div className="rounded-paper border-2 border-border bg-surface-panel p-5 shadow-paper-sm space-y-3">
            ...
            <div
              ...
              className={`cursor-pointer rounded-paper border-2 border-dashed p-6 text-center transition ${
                hayFotoPendiente
        ...
        <form
          onSubmit={guardar}
          className="rounded-paper border-2 border-border bg-surface-panel p-5 shadow-paper-sm space-y-4 xl:col-span-7"
        >
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div>
              ...
              <input
                ...
                inputMode="numeric"
                autoComplete="off"
                placeholder="CC o NIT sin espacios"
                className="w-full rounded-paper border-2 border-border bg-surface-input px-3 py-2 font-mono text-sm outline-none focus:border-accent"
                value={documento}
                onChange={(e) => setDocumento(e.target.value)}
              />
              <p className="mt-1 text-[11px] text-muted">
                Obligatorio. Se imprime en las cuentas de cobro que generes desde tu perfil.
              </p>
            </div>
            <div>
